...
Run the no-side-effect stage immediately after installation:
...
If the host requires a macOS permission prompt, explain the exact permission and why it is needed before asking the user. Refusal leaves the installation at `authorization-required` or `partial`; it does not erase the installed files.

## Report Requirements

The JSON report records the Skill version and path, host and host version, macOS version, architecture, fixture hashes, native helper hashes, completed checks, required authorizations, errors, and next action. Preflight reports are temporary by default. An authorized successful E2E writes a durable receipt under `~/Library/Application Support/folder-icon-replacer/verification/`; this receipt write is part of the disclosed E2E authorization.
